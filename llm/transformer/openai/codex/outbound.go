...
func (t *OutboundTransformer) AllowPassThroughBody(_ context.Context, llmReq *llm.Request, _ *httpclient.Request) bool {
	...
	if requiresGPT6RequestNormalization(llmReq.Model, llmReq.RawRequest.Body) {
		return false
	}
...
func (t *OutboundTransformer) TransformRequest(ctx context.Context, llmReq *llm.Request) (*httpclient.Request, error) {
	...
	if llmReq.RawRequest != nil && llmReq.RawRequest.Headers != nil {
		...
		if !t.isOfficialCodex() || !strings.EqualFold(strings.TrimSpace(rawHeaders.Get(responses.ResponsesLiteHeader)), "true") {
			rawHeaders.Del(responses.ResponsesLiteHeader)
		}
