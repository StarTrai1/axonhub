...
默认不信任转发客户端 IP 的请求头。若 AxonHub 部署在反向代理后，
只配置该代理实际使用的 IP 或网段：
...
此示例适用于本机代理，其他部署应填写真实代理网段。
只有来自可信代理的连接才允许提供 `X-Forwarded-For` 或 `X-Real-IP`；
其他连接使用 TCP 对端地址，防止伪造请求头绕过 IP 限制。不要图方便而信任所有网段。
升级后，在依赖客户端 IP 白名单、黑名单或密钥限制之前，请检查反向代理配置。
程序不会自动修改生产配置。
...
- 客户端 IP 按上述可信代理规则校验，无法使用可信转发头时回退为 TCP 对端地址。
...
密钥 IP 限制与全局控制使用同一套可信代理客户端 IP 解析逻辑，不会仅因请求头存在就信任它。
应将实际反向代理（例如 Nginx、Cloudflare 或 AWS ALB）的地址配置到 `server.trusted_proxies`；
未配置时使用 TCP 对端地址。
