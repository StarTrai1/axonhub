...
渠道操作菜单支持设置按服务器当地时间执行的每日 `HH:MM:SS` 计划。计划时间到达时，服务必须运行且主机不能休眠；关闭浏览器不会停止调度器。弹窗展示下次计划、最近调度扫描、上次计划时间、实际开始时间、默认测试模型和执行结果。日期按浏览器当地时区显示。

每次检查沿用渠道连接与代理设置，发出一次轻量真实模型请求，并设置两分钟超时。请求消耗上游用量，但不会重置配额，也不保证启动 5 小时窗口；已运行的上游窗口不会因健康检查而重新开始。不要仅因四舍五入后的使用百分比仍为零就增加 token 消耗。

空闲时调度器最多每分钟保存一次进度，并在联系上游前记录本次执行。服务重启或主机恢复后，每个渠道只补最近一个漏过的计划，不逐天、逐时段回放积压任务。已记录但未完成的执行显示为**进程中断，结果未确认**，不会自动重复请求，避免崩溃后重复消耗。首次启动没有检查点时从当前时间开始跟踪，不伪造历史执行；检查点写入失败时暂停发起新的检查。

最近 100 条完成结果和各渠道最近一次执行在重启后保留，包括尚未生成普通请求记录就失败的情况。实际测试请求与用量仍可在请求历史查看。此检查点用于单个调度实例的重启恢复，不是多个服务共享数据库时的分布式执行锁。
