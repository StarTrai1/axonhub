...
test('channel and retry-policy GraphQL selections load the new auto-disable fields', () => {
  ...
  for (const selection of policySelections) {
    // Channel-list rows intentionally select only routingTier; dialogs fetch
    // a complete snapshot before reading or editing availability policies.
    if (!/stream|apiKeyAutoDisableMode/.test(selection)) {
      assert.match(selection, /policies\s*\{\s*routingTier\s*\}/);
      continue;
    }
