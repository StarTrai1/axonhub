...
test('normalized quota does not restore the removed duplicate quota column', () => {
  assert.doesNotMatch(columnsSource, /id:\s*['"]quota['"]/);
  assert.match(columnsSource, /CodexUsageCell channel=\{row.original\}/);
});
...
test('channel schemas preserve normalized quota for every supported provider', () => {
...
test('normalized parser preserves every limit without a five-row cap', () => {
  const limits = parseQuotaLimits({
    _limits: [
      { type: 'token', window: '5h', usageRatio: 0.1, status: 'available' },
