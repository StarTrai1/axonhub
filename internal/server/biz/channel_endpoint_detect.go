		...
		go func(index int, format string) {
			...
			defer func() {
				if recovered := recover(); recovered != nil {
					results[index] = DetectedChannelEndpoint{APIFormat: format, Reason: endpointDetectReasonUnreachable}
					log.Error(ctx, "endpoint detection: probe panicked",
						log.Int("channel_id", entity.ID),
						log.String("api_format", format),
