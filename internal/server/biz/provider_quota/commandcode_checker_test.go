...
func TestCommandCodeRealWirePayload(t *testing.T) {
	// Preserve the captured wire shape while keeping resets in the future;
	// normalization intentionally discards expired reset timestamps.
	now := time.Now().UTC()
	fiveHourReset := now.Add(5 * time.Hour).Truncate(time.Millisecond)
	weeklyReset := now.Add(7 * 24 * time.Hour).Truncate(time.Millisecond)
	...
	require.Equal(t, fiveHourReset.UnixMilli(), quota.Limits[0].NextResetAt.UnixMilli())
	require.NotNil(t, quota.Limits[1].NextResetAt)
	require.Equal(t, weeklyReset.UnixMilli(), quota.Limits[1].NextResetAt.UnixMilli())
