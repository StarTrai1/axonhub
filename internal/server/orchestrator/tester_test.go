...
func TestBuildChannelTestRequestResponsesCompatibility(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			request, err := buildChannelTestRequest(test.model, test.requestedStream)
			...
			require.Equal(t, channelTestPrompt, gjson.GetBytes(request.Body, "messages.0.content").String())
			...
			require.NotContains(t, string(request.Body), "Hello world")
			require.NotContains(t, string(request.Body), "helpful assistant")
