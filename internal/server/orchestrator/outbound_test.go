...
func TestPersistentOutboundTransformer_CanRetry_429_WithRetryAfter(t *testing.T) {
	...
	t.Run("429 with bounded Retry-After retries the last channel", func(t *testing.T) {
		outbound := &PersistentOutboundTransformer{
			wrapped: &mockTransformer{},
			state: &PersistenceState{
				CurrentCandidate: &ChannelModelsCandidate{
	...
	t.Run("429 with multiple headers honors bounded Retry-After", func(t *testing.T) {
		outbound := &PersistentOutboundTransformer{
			wrapped: &mockTransformer{},
			state: &PersistenceState{
				CurrentCandidate: &ChannelModelsCandidate{
...
func TestPersistentOutboundTransformer_CanRetry_429_WithoutRetryAfter(t *testing.T) {
	...
	t.Run("429 with nil headers permits bounded retry", func(t *testing.T) {
		outbound := &PersistentOutboundTransformer{
			wrapped: &mockTransformer{},
			state: &PersistenceState{
				CurrentCandidate: &ChannelModelsCandidate{
	...
	t.Run("429 with empty headers permits bounded retry", func(t *testing.T) {
		outbound := &PersistentOutboundTransformer{
			wrapped: &mockTransformer{},
			state: &PersistenceState{
				CurrentCandidate: &ChannelModelsCandidate{
	...
	t.Run("429 without Retry-After permits bounded retry", func(t *testing.T) {
		outbound := &PersistentOutboundTransformer{
			wrapped: &mockTransformer{},
			state: &PersistenceState{
				CurrentCandidate: &ChannelModelsCandidate{
...
func TestPersistentOutboundTransformer_CanRetry_429_WithMultipleModels(t *testing.T) {
	...
	t.Run("429 permits bounded retry with multiple models on last channel", func(t *testing.T) {
		outbound := &PersistentOutboundTransformer{
			wrapped: &mockTransformer{},
			state: &PersistenceState{
				CurrentCandidate: &ChannelModelsCandidate{
...
func TestPersistentOutboundTransformer_RetrySelectionCounts(t *testing.T) {
	...
	// Local compatibility retries keep the current model but still make an
	// upstream attempt, so they must participate in the same load window.
	processor.state.responsesRejectedStatusRetryChannel = second.Channel.ID
	require.NoError(t, processor.PrepareForRetry(ctx))
	assertCounts(3, 3)
	processor.state.responsesLiteWebSearchRetryChannel = second.Channel.ID
	require.NoError(t, processor.PrepareForRetry(ctx))
	assertCounts(3, 4)

	processor.state.CurrentCandidateIndex = 0
	processor.state.CurrentCandidate = first
	require.NoError(t, processor.NextAlternativeChannel(ctx))
	assertCounts(3, 5)
	require.Error(t, processor.NextAlternativeChannel(ctx))
	assertCounts(3, 5)
}
