...
func (s *LoadBalancedSelector) pinStickyCandidate(
	...
	if candidate, _ := extractStickyCandidate(highestRoutingTierCandidates(candidates), channelID); candidate == nil {
	...
	for _, groupKey := range priorities {
		...
		if s.retainManualSwitchCandidate {
			...
			if loadBalancer == nil {
				...
				sortedCandidates = loadBalancer.SortWithoutTrackingLimit(ctx, group, req.Model, useStream, limit)
			}
