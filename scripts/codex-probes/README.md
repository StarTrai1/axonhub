...
运行要求：Linux（使用 `/proc`、进程组和 `flock`），Python 3.11+，Codex CLI 0.155.1+。本实现已核对官方 `rust-v0.155.1` → `rust-v0.157.1` 源码，兼容 0.156.1、0.157.0 和 0.157.1。升级后先运行 `--check` 并检查官方变更。Python 仅使用标准库。
...
- GitHub Actions 用 fake CLI 验证新会话、清理、超时/中断、错误分类、阶段切换、计数、定时去重和不触碰外部文件；hosted matrix 固定下载官方 0.157.0 和 0.157.1，分别使用 gpt-6-sol、gpt-6-luna，仅连接 loopback 模拟服务器验证真实 CLI 五并发与取消，不调用真实上游。离线测试不能证明提供方额度窗口行为或封禁策略。
...
精确 tag 源码差异集中在 Windows daemon、Code Mode 和本地 MCP 进程启动（隐藏控制台窗口、处理 Job Object 和关闭继承的启动器 stdio）。`codex exec` 参数、JSONL 事件、Responses 协议和 Sol/Luna 模型目录没有变化，Linux 调度、隔离会话和暂停清理逻辑无需修改。CI 保留 0.157.0 基线并验证 0.157.1；仅使用 loopback 模拟服务，不代表真实上游可用性验证。
